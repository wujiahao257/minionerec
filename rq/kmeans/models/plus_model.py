...
class RQKMeansPlusModel(nn.Module):
    """RQ-Kmeans+ 内部的量化网络，独立维护编码器、码本和解码器；无高斯先验采样。
    ...
    def __init__(self,
        ...
        """初始化 RQKMeansPlusModel：通过 MLP 编码、残差量化与 MLP 解码学习商品离散表示；此实现无高斯先验采样。

        Args:
            self (RQKMeansPlusModel): 当前实例，由 Python 在调用实例方法时自动传入。
            in_dim (int): 商品连续向量的特征维度 D。
            num_emb_list (list[int] | None): 每层码本大小列表，长度决定量化层数；构造模型时需要实际列表。
            e_dim (int): 单个码本向量及量化器输入的末维大小 d，须与 encoder 输出维度一致。
            layers (list[int] | None): MLP 的层宽列表；RQKMeansPlusModel 中只传中间层，MLPLayers 中包含首尾维度。
